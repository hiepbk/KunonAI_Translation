#python final-gpu.py --file ./sample_data/2.2.4 의료비 지출내역.pdf --ocr paddle --model gpt-4o --mode all 2 --ocr-view
        ...
        if ocr_type == 'paddle':
            ...
            if ocr_ar is not None:
                ...
                if translation_mode == 'eng_chi':
                    ...
                    # 한국어 OCR 결과 수집
                    texts_ko, bboxes_ko, scores_ko = parse_paddleocr_result(result_ko)
                    for text, bbox, score in zip(texts_ko, bboxes_ko, scores_ko):
                        try:
                            if hasattr(bbox, 'tolist'):
                                bbox = bbox.tolist()
                            x_coords = [float(p[0]) for p in bbox if len(p) >= 2]
                    ...
                    # 중국어 OCR 결과 수집
                    texts_other, bboxes_other, scores_other = parse_paddleocr_result(result_other)
                    for text, bbox, score in zip(texts_other, bboxes_other, scores_other):
                        try:
                            if hasattr(bbox, 'tolist'):
                                bbox = bbox.tolist()
                            x_coords = [float(p[0]) for p in bbox if len(p) >= 2]
                ...
                # 두 OCR 결과를 all_texts와 all_bboxes로 직접 변환
                # 병합 수행 (점수는 더미로 처리)
                dummy_scores = [1.0] * len(all_rec_texts)
                all_rec_texts, all_dt_polys, _ = merge_ocr_results(all_rec_texts, all_dt_polys, dummy_scores)
                
                for i, text in enumerate(all_rec_texts):
                    if not text or not text.strip():
                        continue
                    
                    bbox = all_dt_polys[i] if i < len(all_dt_polys) else None
                    if bbox is not None and len(bbox) > 0:
                        try:
                            if hasattr(bbox, 'tolist'):
                                bbox = bbox.tolist()
        ...
        if ocr_type == 'paddle':
            ...
            if ocr_ar is not None:
                ...
                if translation_mode == 'all':
                    ...
                    result_arabic = ocr_ar.ocr(tmp_path, cls=True)
                ...
                if translation_mode == 'eng_chi':
                    ...
                    # 한국어 OCR 결과 수집
                    texts_ko, bboxes_ko, scores_ko = parse_paddleocr_result(result_ko)
                    for i, (text, bbox, score) in enumerate(zip(texts_ko, bboxes_ko, scores_ko)):
                        try:
                            if hasattr(bbox, 'tolist'):
                                bbox = bbox.tolist()
                            x_coords = [float(p[0]) for p in bbox if len(p) >= 2]
                    ...
                    # 아랍어 OCR 결과 수집
                    texts_other, bboxes_other, scores_other = parse_paddleocr_result(result_other)
                    for i, (text, bbox, score) in enumerate(zip(texts_other, bboxes_other, scores_other)):
                        try:
                            if hasattr(bbox, 'tolist'):
                                bbox = bbox.tolist()
                            x_coords = [float(p[0]) for p in bbox if len(p) >= 2]
                ...
                # 병합 수행 (전체 뷰)
                all_rec_texts, all_dt_polys, all_rec_scores = merge_ocr_results(all_rec_texts, all_dt_polys, all_rec_scores)
                ...
                for i, text in enumerate(all_rec_texts):
                    ...
                    bbox = all_dt_polys[i] if i < len(all_dt_polys) else None
                    score = all_rec_scores[i] if i < len(all_rec_scores) else 1.0
        ...
        if ocr_type == 'paddle':
            ...
            if result and len(result) > 0:
                ...
                if isinstance(res, dict):
                    ...
                    # dict 형식인 경우 병합 수행
                    rec_texts, dt_polys, rec_scores = merge_ocr_results(rec_texts, dt_polys, rec_scores)
                elif isinstance(res, list):
                    # PaddleOCR standard format
                    # parse_paddleocr_result 호출 (내부에서 merge_ocr_results 호출됨)
                    rec_texts, dt_polys, rec_scores = parse_paddleocr_result([res])
...
# EasyOCR이 필요할 때만 import 시도
if OCR_ENGINE == 'easyocr':
    if not import_easyocr():
        print("EasyOCR이 설치되지 않았습니다. 설치: pip install easyocr")
...
if FULL_VIEW_MODE or OCR_VIEW_MODE:
    ...
    if OCR_ENGINE == 'paddle':
        ...
        if TRANSLATION_MODE == 'eng_only':
            ...
            ocr = PaddleOCR(
                ...
                det_db_unclip_ratio=5,  # 텍스트 영역 확장 비율 증가 (기본값 1.5 -> 3.0, 더 긴 텍스트 인식)
                det_db_box_thresh=0.1,  # 텍스트 박스 임계값 감소 (기본값 0.6 -> 0.4, 더 많은 텍스트 영역 감지)
                det_db_thresh=0.2,  # 텍스트 감지 임계값 감소 (기본값 0.3 -> 0.2, 더 많은 텍스트 감지)
                det_limit_side_len=1920,  # 이미지 크기 제한 증가 (기본값 960 -> 1920, 더 정확한 인식)
                max_text_length=10  # 최대 텍스트 길이 증가 (기본값 25 -> 100, 더 긴 문장 인식)
            )
    ...
    if TRANSLATION_MODE == 'eng_only':
        ...
        ocr = PaddleOCR(
            ...
            det_db_unclip_ratio=5,  # 텍스트 영역 확장 비율 증가 (기본값 1.5 -> 2.0, 더 긴 텍스트 인식)
            det_limit_side_len=1920,  # 이미지 크기 제한 증가 (기본값 960 -> 1920, 더 정확한 인식)
            max_text_length=50  # 최대 텍스트 길이 증가 (기본값 25 -> 50, 더 긴 문장 인식)
        )
